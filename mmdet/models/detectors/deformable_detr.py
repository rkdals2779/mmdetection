...
class DeformableDETR(DetectionTransformer):
    ...
    def pre_transformer(
        ...
        # import cv2
        # import numpy as np
        # w, h = encoder_inputs_dict["spatial_shapes"][0]
        # bi_mask = encoder_inputs_dict["feat_mask"].cpu().numpy()[0, :h*w].reshape(w, h)
        # mask = np.uint8(bi_mask) * 255
        # bi_mask_ = encoder_inputs_dict["feat_mask"].cpu().numpy()[1, :h * w].reshape(w, h)
        # mask_ = np.uint8(bi_mask_) * 255
        # print("mask unique: ", np.unique(bi_mask))
        #
        # w2, h2 = encoder_inputs_dict["spatial_shapes"][1]
        # bi_mask2 = encoder_inputs_dict["feat_mask"].cpu().numpy()[0, h * w:h * w+h2 * w2].reshape(w2, h2)
        # mask2 = np.uint8(bi_mask2) * 255
        # bi_mask_2 = encoder_inputs_dict["feat_mask"].cpu().numpy()[1, h * w:h * w+h2 * w2].reshape(w2, h2)
        # mask_2 = np.uint8(bi_mask_2) * 255
        #
