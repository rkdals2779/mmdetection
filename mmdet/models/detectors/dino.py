...
class DINO(DeformableDETR):
    ...
    def forward_transformer(
        ...
        encoder_inputs_dict, decoder_inputs_dict = self.pre_transformer(
            mlvl_feats=img_feats, batch_data_samples=batch_data_samples)
    ...
    def pre_decoder(
        ...
        query = self.query_embedding.weight[:, None, :]     # (nmQ, 1, C)
        query = query.repeat(1, bs, 1).transpose(0, 1)      # (B, nmQ, C)
        if self.training:
            dn_label_query, dn_bbox_query, dn_mask, dn_meta = \
                self.dn_query_generator(batch_data_samples)
            query = torch.cat([dn_label_query, query], dim=1)   # (B, Q, C)
